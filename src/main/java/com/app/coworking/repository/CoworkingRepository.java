...
public interface CoworkingRepository extends JpaRepository<Coworking, Long> {
    boolean existsByName(@NotBlank(message = "Coworking name is required")
                         @Size(min = 2, max = 100,
                                 message = "Coworking name must be between 2 and 100 characters")
                         String name);

    boolean existsByAddress(@NotBlank(message = "Address is required")
                            @Size(min = 10, max = 200,
